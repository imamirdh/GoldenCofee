...
import { HiOutlineDocumentText, HiMenu, HiOutlineMail } from "react-icons/hi";

import { MdMenuOpen, MdOutlineLocalOffer } from "react-icons/md";
...
function MobileMenuSidebar() {
  ...
  return (
    <div
      className={
        contextdata.isDark
          ? " lg:hidden dark w-screen h-screen fixed top-0 bg-opacity-50 bg-black z-50"
          : " lg:hidden w-screen h-screen fixed top-0 bg-opacity-50 bg-black z-50"
      }
      dir="rtl"
    >
      <div
        className={
          menubox
            ? "opacity-1 transition-all duration-300  sidebar h-screen px-2 w-4/6 overflow-y-auto text-center bg-zinc-800  dark:bg-zinc-900 text-orange-200 font-Dana"
            : " w-4/6 h-screen opacity-0 translate-x-[100%] transition-all duration-300  "
        }
        // className={
        //   menubox
        ...
        // }
      >
         <div class="h-[10%] p-2.5 flex items-center">
        <svg
          width="100"
          height="40"
          viewBox="0 0 138 55"
        ...
        <p className="ms-2 text-xl font-MorabbaMedium text-zinc-500">داشبورد</p>
      </div>
      <div class="mb-2 bg-zinc-500 h-[1px]"></div>
        ...
        onClick={() => {
          closeSidebar();
          contextdata.setIsDark((prev) => !prev)
        }}
        ...
        >
          ...
          <span class="text-[15px] mr-2  font-bold">
            {contextdata.isDark ? "تم روشن" : "تم تیره"}
          </span>
        </button>
        <button class="p-2.5 mt-3 w-full flex items-center rounded-md rounded-l-full px-4 duration-300 cursor-pointer hover:bg-zinc-600 hover:text-red-400">
